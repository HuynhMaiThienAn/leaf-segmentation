...
def apply_5d_pca(path):
    ...
    # Update the markdown cell for Steps 3 & 4 to describe the 5D PCA
    for i, cell in enumerate(cells):
        if cell.get("cell_type") == "markdown":
            src = "".join(cell.get("source", []))
            if "CIELAB-PCA Projection" in src or "Color PCA Projection" in src:
                new_md = [
...
def main():
    apply_5d_pca("leaf_segmentation_tutorial_clean.ipynb")
    apply_5d_pca("leaf_segmentation_tutorial.ipynb")
